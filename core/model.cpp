...
Model::Model(const char* filename, bool is_skybox, bool is_from_mmd)
	:is_skybox(is_skybox), is_from_mmd(is_from_mmd),diffusemap(nullptr),normalmap(nullptr),specularmap(nullptr),roughnessmap(nullptr),metalnessmap(nullptr),occlusion_map(nullptr),emision_map(nullptr),environment_map(nullptr),glossiness_map(nullptr)
{
	std::ifstream in;
...
Model::~Model() 
{
	...
	delete glossiness_map; glossiness_map = nullptr;
...
void Model::create_map(const char* filename)
{
	...
	//加载光泽度贴图
	texfile = texfile.substr(0, dot) + std::string("_glossiness.tga");
	if (_access(texfile.data(), 0) != -1)
	{
		occlusion_map = new TGAImage();
		load_texture(filename, "_glossiness.tga", metalnessmap);
	}
}
...
std::vector<int> Model::face(int idx) 
{
	std::vector<int> face;
	for (int i = 0; i < 3; i++) 
		face.push_back(faces[idx][(double)i*3]);
	return face;
}
...
{
	return verts[faces[iface][(double)nthvert*3]];
}

vec2 Model::uv(int iface, int nthvert) 
{
	return uvs[faces[iface][(double)nthvert * 3+1]];
}

vec3 Model::normal(int iface, int nthvert) 
{
	int idx = faces[iface][(double)nthvert * 3+2];
...
{
	std::string texfile(filename);
	size_t dot = texfile.find_last_of(".");//写size_t是为了可移植型，一般情况都可以使用unsigned int来代替
...
float Model::glossiness(vec2 uv)
{
	uv[0] = fmod(uv[0], 1);
	uv[1] = fmod(uv[1], 1);
	int uv0 = uv[0] * glossiness_map->get_width();
	int uv1 = uv[1] * glossiness_map->get_height();
	return roughnessmap->get(uv0, uv1)[0] / 255.f;
}
